...
class Trainer(BaseTrainer):
    ...
    def predict(self, batch, unsort=True):
        ...
        deprel_prob_seqs = [[preds[2][i][j+1] for j, h in enumerate(hs)] for i, hs in enumerate(head_seqs)]

        pred_tokens = [[[str(head_seqs[i][j]), deprel_seqs[i][j], deprel_prob_seqs[i][j]] for j in range(sentlens[i]-1)] for i in range(batch_size)]
        if unsort:
